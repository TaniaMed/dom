...
let text = contacts.reduce(function(previousValue, currentValue) {
    return previousValue + `<li data-email="${currentValue.email}" data-phone="${currentValue.phone}"><strong>${currentValue.name}</strong></li>`},'');

window.addEventListener('load', function() {
    const contactsList = document.querySelector('.contacts-list');
    contactsList.innerHTML = text;
});
